...
server_files=/app/serverfiles
persistent_data=/app/data

mkdir -p /home/steam/.steam/ 2>/dev/null
chmod -R 777 /home/steam/.steam 2>/dev/null
echo " "
...
if [ ! -f "$server_files/app.cfg" ]; then
    ...
    if grep -q "server_persistent_data_override_folder" $server_files/app.cfg; then
        sed -i "/server_persistent_data_override_folder=/c server_persistent_data_override_folder=/app/data" $server_files/app.cfg
    else
        echo -ne '\nserver_persistent_data_override_folder=/app/data' >> $server_files/app.cfg
    fi
fi
echo " "
